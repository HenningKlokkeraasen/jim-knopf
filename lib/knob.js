var Knob;
Knob = function(input, ui, changeCallback, inputCallback) {
...
Knob.prototype = {
  ...
  _updateWhileMoving: function(event) {
    ...
    var value = parseFloat(this.min + range * percent);

    var step = (this.settings.max - this.min) / range;
    this.value = this.input.value = (value / parseFloat(step)) * parseFloat(step);
  ...
  changed: function(direction) {
    this.input.value = parseFloat(this.limit(parseFloat(this.input.value) + direction * (this.input.step || 1)));
  ...
  _getSettings: function(input) {
    var settings = {
      ...
      value: parseFloat(input.value) || parseFloat(input.min),
      angleoffset: 0,
      anglerange: 360
    };
    settings.range = parseFloat(settings.max) - parseFloat(settings.min);
